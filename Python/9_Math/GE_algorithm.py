...
def main_task():
    ...
    for puck in list_pucks:
        ...
        current_puck.append(puck["飞机转场记录号"])
        current_puck.append(resource_period["begin_index"])
        current_puck.append(resource_period["end_index"])
        ...
        for gate in list_gates: 
            ...
            # 分配完毕，下一个puck
            if puck["是否分配"] == 1:
                current_puck.append(gate["登机口"])
                break
        if puck["是否分配"] == 0:
            current_puck.append("NONE")
